...
function cplanilla(){
    ...
    document.getElementById("ingresodatos").style.display = "none"
}

function save_data(){
    var nombre = document.getElementById("namecampos").value
    var tipocc = document.getElementById("tdoc").value
    var numcc = document.getElementById("ccnumer").value
    localStorage.setItem('nombre', nombre)
    localStorage.setItem('tipocc', tipocc)
    localStorage.setItem('numcc',numcc)
}



function elecciondetiquete(result){
    var nombre = localStorage.getItem("nombre")
    if (nombre == null){
        
        imprimirsindatos(result)
    }else{
        imprimir(result)
...
function imprimirsindatos(response){
    ...
    doc.setFontSize(12);
    doc.setFontType("bold")
    doc.text(transurimp, 1.6, 9, { align: "center" });
    ...
    doc.text(variableorigen , 1.8,4.5, { align: "center" });
    doc.text(destinofianal, 2,5, { align: "center" });
    ...
    doc.setFontSize(10);
    doc.setFontType("bold")
    doc.text(corigenimp,0.5, 4.5, { align: "center" } );
    doc.text(cdestinoimp,0.5, 5, { align: "center" });
    
    
    
    doc.setFontSize(12);
    doc.setFontType("bold")
    doc.text(totalimp,0.5, 5.8, { align: "center" });
    doc.text(cvalorfinal, 0.4,6.3, { align: "center" });
    
    doc.setFontSize(8);
    doc.setFontType("bold")
    doc.text(ccdesimp,0.5, 7, { align: "center" });
